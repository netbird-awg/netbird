...
func (s *Service) enrichFleetDMHostsWithHealth(
	...
	healthByHost := make([]map[string]any, len(hosts))
	...
	for index, host := range hosts {
		...
		hostIDs[index] = strconv.Itoa(hostID)
	}
	...
	for index, hostID := range hostIDs {
		...
		group.Go(func() error {
			request, err := s.newProviderRequest(
				groupCtx,
				http.MethodGet,
				config.APIURL+"/api/v1/fleet/hosts/"+url.PathEscape(hostID)+"/health",
				nil,
			)
			if err != nil {
				return fmt.Errorf("create FleetDM host %s health request: %w", hostID, err)
			}
			request.Header.Set("Authorization", "Bearer "+config.APIToken)
			...
			if err := s.doProviderJSON(request, &response); err != nil {
				return fmt.Errorf("get FleetDM host %s health: %w", hostID, err)
			}
			if len(response.Health) == 0 {
				return fmt.Errorf("FleetDM host %s returned no health data", hostID)
			}
			responseHostID := scalarString(response.HostID)
			if responseHostID != "" && responseHostID != hostID {
				return fmt.Errorf("FleetDM host %s returned health data for host %s", hostID, responseHostID)
			}
			healthByHost[index] = response.Health
			return nil
		})
	}
	...
	for index, health := range healthByHost {
		for key, value := range health {
			hosts[index][key] = value
		}
	}
	return nil
}
