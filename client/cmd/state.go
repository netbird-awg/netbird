...
var stateListCmd = &cobra.Command{
	...
	Example: "  netibird-awg state list",
	RunE:    stateList,
}
...
This will perform cleanup operations and remove the state.`,
	Example: `  netibird-awg state clean dns_state
  netibird-awg state clean --all`,
	RunE: stateClean,
...
This will remove the state without performing any cleanup operations.`,
	Example: `  netibird-awg state delete dns_state
  netibird-awg state delete --all`,
	RunE: stateDelete,
	PreRunE: func(cmd *cobra.Command, args []string) error {
		// Check mutual exclusivity between --all flag and state-name argument
		if allFlag && len(args) > 0 {
